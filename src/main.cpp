...
bool CTxOut::IsDust() const
{
    ...
    // so dust is a txout less than 54 uPMC
...
void SetGenesisHash() {
        // Genesis Block:
        // CBlock(hash=000000000019d6, ver=1, hashPrevBlock=00000000000000, hashMerkleRoot=4a5e1e, nTime=1390363105, nBits=1d00ffff, nNonce=2083236893, vtx=1)
...
unsigned char pchMessageStart[4] = { 0x95, 0x03, 0x64, 0xd1 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PremineMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PremineMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("PremineMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("premine-miner");
    ...
    try { loop {
        ...
        printf("Running PremineMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PremineMiner terminated\n");
        throw;
    }
}
