...
{
    QString ret = QString("premine:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non PMC unit input in lnReqAmount, we generate the URI with PMC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::PMC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
